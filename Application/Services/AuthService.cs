...
public class AuthService : IAuthService
    {
        ...
        public async Task<AuthApiResponse> LoginAsync(LoginRequest loginRequest, string traceId)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("[{TraceId}] Login failed for user {Email}. Status: {StatusCode}, Response: {Response}", 
        ...
        public async Task<AuthApiResponse> RegisterAsync(RegisterRequest registerRequest, string traceId)
        {
            try
            {
                _logger.LogInformation("[{TraceId}] Attempting registration for user {Email}", traceId, registerRequest.Email);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    _logger.LogInformation("[{TraceId}] Registration successful for user {Email}", traceId, registerRequest.Email);
                    return authResponse ?? new AuthApiResponse();
                }
                else
                {
                    _logger.LogWarning("[{TraceId}] Registration failed for user {Email}. Status: {StatusCode}, Response: {Response}", 
            ...
            {
                _logger.LogError(ex, "[{TraceId}] Error during registration for user {Email}", traceId, registerRequest.Email);
                return new AuthApiResponse
                {
                    Completion = new Completion
                    {
